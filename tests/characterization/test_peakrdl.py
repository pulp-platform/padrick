...
"""Smoke test for the optional SystemRDL/PeakRDL register-file backend.

Skips cleanly when the peakrdl extra is not installed in the running venv.
"""
...
def test_peakrdl_rtl_backend(tmp_path: Path):
    ...
    result = run_padrick(["generate", "rtl", "--register-backend", "peakrdl",
                          "-o", str(tmp_path), str(SAMPLE_CONFIG)])
...
def test_peakrdl_rtl_omits_hardwired_pad(tmp_path: Path):
    ...
    result = run_padrick(["generate", "rtl", "--register-backend", "peakrdl",
                          "--no-version-string", "-o", str(tmp_path), str(QUASI_STATIC_CONFIG)])
    assert result.returncode == 0, result.stderr
    rdl = (tmp_path / "src" / "quasi_static_padframe_aon_regs.rdl").read_text()
