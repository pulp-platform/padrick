...
def rtl(generator_settings: GeneratorSettings, config_file: str, output: str, header, version_string, register_backend):
    ...
    if register_backend == "peakrdl":
        if padframe.config_interface != ConfigInterface.regbus:
            raise ClickException("config_interface handling for the peakrdl register backend lands with its "
                                 "padframe integration; use the reggen backend")
        if padframe.config_port_topology != ConfigPortTopology.shared:
            raise ClickException("config_port_topology handling for the peakrdl register backend lands with "
                                 "its padframe integration; use the reggen backend")
...
def driver(generator_settings: GeneratorSettings, config_file: str, output: str, header, version_string, register_backend):
    ...
    if not padframe:
        raise UsageError("Failed to parse the configuration file")
    if register_backend == "peakrdl" and padframe.config_port_topology != ConfigPortTopology.shared:
        raise ClickException("config_port_topology handling for the peakrdl register backend lands with "
                             "its padframe integration; use the reggen backend")
    header_sections = []
    if version_string:
